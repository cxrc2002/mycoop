...
def train():
    ...
    print(f"clip_model created successfully, device: {next(clip_model.parameters()).device}")
    textencoder = TextEncoder(clip_model).to(device)
    print(f"textencoder created successfully, {next(textencoder.parameters()).device}")
    ...
    orgctx = torch.randn(total_num, n_ctx, 512).to(clip_model.dtype).to(device)
    prompt_prefix = " ".join(["X"] * n_ctx)
    prompt_prefix = prompt_prefix + "."
    tokenized_prefix = clip.tokenize(prompt_prefix).to(device)
    embedding = clip_model.token_embedding(tokenized_prefix).type(clip_model.dtype)  # (1,77,512)
    embedding = embedding.expand(batch_size, -1, -1)   # (batch_num, 77, 512)

    for i in range(epoch):
        print(f"epoch: {i+1}")
        for j in range(batch_num):
            batch_ctx = orgctx[batch_size*j:batch_size*(j+1), :, :]
            prompts = torch.cat(
                            [
                                embedding[:, :1, :],  # (n, n_ctx, dim)  start token
                                batch_ctx,     # (n, n_ctx, dim)
                                embedding[:, 1+n_ctx:, :],  # (n, *, dim)   end token
                            ],
                            dim=1,
                        )
            prompts = prompts.to(device)
            # print(prompts.shape)
            encoded_text_features = textencoder(prompts, n_ctx)                  # 10,1024
            encoded_text_features = encoded_text_features.unsqueeze(1)     # 10,1,1024
            ...
            # 计算损失
            loss = criterion(decoded_output, batch_ctx)
            ...
            print(f"Epoch [{i + 1}/{epoch}],Batch: [{j+1}/{batch_num}], Loss: {loss.item():.4f}")

    torch.save(imgdecoder.state_dict(), f"E:/mycoop/checkpoints/imgdecoder_nctx{n_ctx}_batchsize{batch_size}_ep{epoch}.pth")
